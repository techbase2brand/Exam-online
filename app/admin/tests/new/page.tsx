...
import { Plus, Trash2, Save, ArrowLeft, Type, CheckSquare, List, AlignLeft, AlertCircle, Trophy } from 'lucide-react'
      ...
      if (q.id === questionId) {
        return {
          ...
          options: q.options.map(o => {
            if (o.id === optionId) {
              // If it's a radio (MCQ), uncheck others if this is checked
              if (q.question_type === 'mcq' && updates.is_correct) {
                return { ...o, ...updates }
              }
              return { ...o, ...updates }
            }
            // For MCQ, uncheck others
            if (q.question_type === 'mcq' && updates.is_correct) {
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-32">
      ...
      <div className="fixed top-0 right-0 left-64 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100 px-8 py-4 shadow-sm flex justify-between items-center transition-all">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-xl font-black text-gray-800 tracking-tight">New Test</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {questions.map((question, qIndex) => (
            <div key={question.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Marks for this question</label>
                  <input
                    required
                    type="number"
                    min="1"
        ...
        <div className="flex justify-end gap-4 border-t pt-8">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex items-center gap-2 bg-blue-600 text-white px-8 py-2 rounded-lg hover:bg-blue-700 transition-colors font-bold disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-200"
          >
            <Save size={20} />
            {loading ? 'Creating Test...' : 'Save Test & Publish'}
          </button>
        </div>
      </form>
    </div>
